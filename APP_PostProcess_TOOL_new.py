...
#FOR TESTING
Version = "TESTING"
sdFile = "C:\\onx-syncs\\sd-files\\live\\20171017_Ohio_171.sd"
...
appDirectory = sdFile.split("sd-files",1)[0:-1]
landHandoffDirectory = appDirectory[0]+"handoff\\lands\\"
gmuHandoffDirectory = appDirectory[0]+"handoff\\gmus\\" #for future gmu coying
stagingDirectory ="D:\\"
oldExtractsDirectory = "D:\\extracted\\" #eventually replace with unzipping code block that unzips to a last publish directory
sdFolderExtractName = (sdFile.split('\\')[-1]).split(".")[0]
templateDirectory = appDirectory[0]+"automation\\mastertemplate\\"
...
if arcpy.Exists(oldExtractsDirectory+sdFolderExtractName):
	oldPubList = os.listdir(oldExtractsDirectory)
	for oldPub in sorted(oldPubList,reverse=True):
##		print "this is the old pub "+oldPub
##		print "this is the extracted sd file folder name "+(sdFile.split('\\')[-1]).split(".")[0]
		if oldPub == sdFolderExtractName:
			print "we got a match "+oldPub +" and "+sdFolderExtractName
##		if oldPub == sdFile.split("'\'")[-1]:
