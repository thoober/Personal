...
OldePublish_SectionLines = OldePublishGDBpath+'/PLSS/Section_Lines'
OldePublish_BoundaryLines = OldePublishGDBpath+'/PLSS/Section_Boundaries' #variable added 092016 to handle Boundary used in old data
Handoff_SectionLines  = handoffGDBpath+'/Sections/Section_Boundaries'
...
if arcpy.Exists(OldePublish_SectionLines):
	...
	arcpy.FeatureClassToFeatureClass_conversion(OldePublish_SectionLines, handoffGDBpath+'/Sections/','SectionBoundaries')
...
if arcpy.Exists(OldePublish_BoundaryLines):    #code block added 092016 to handle Boundary used in old data
	# Create a feature dataset in the HandoffGDB named 'Sections' into which section areas and boundaries from the OldePublishGDB are going to be clipped
	arcpy.CreateFeatureDataset_management(handoffGDBpath, "Sections", WebMercator)
	arcpy.FeatureClassToFeatureClass_conversion(OldePublish_BoundaryLines, handoffGDBpath+'/Sections/','SectionBoundaries')
	arcpy.AddMessage('/"Sections" feature dataset created')
...
OldePublish_SectionAreas = OldePublishGDBpath+'/PLSS/Section_Areas'
Handoff_SectionAreas = handoffGDBpath+'/Sections/Section_Areas'

if arcpy.Exists(OldePublish_SectionAreas):
	arcpy.CreateFeatureDataset_management(handoffGDBpath, "Sections", WebMercator)
	arcpy.AddMessage('/"Sections" feature dataset created')
	arcpy.FeatureClassToFeatureClass_conversion(OldePublish_SectionAreas, handoffGDBpath+'/Sections/','SectionAreas')
...
OldePublish_SectionLines2 = OldePublishGDBpath+'/Sections/Section_Lines'
OldePublish_BoundaryLines2 = OldePublishGDBpath+'/Sections/Section_Boundaries' #variable added 092016 to handle Boundary used in old data
Handoff_SectionLines2  = handoffGDBpath+'/Sections/Section_Boundaries'

if arcpy.Exists(OldePublish_SectionLines2):
	# Create a feature dataset in the HandoffGDB named 'Sections' into which section areas and boundaries from the OldePublishGDB are going to be clipped
	arcpy.CreateFeatureDataset_management(handoffGDBpath, "Sections", WebMercator)
	arcpy.AddMessage('/"Sections" feature dataset created')
	arcpy.FeatureClassToFeatureClass_conversion(OldePublish_SectionLines2, handoffGDBpath+'/Sections/','SectionBoundaries')
...
if arcpy.Exists(OldePublish_BoundaryLines2):    #code block added 092016 to handle Boundary used in old data
	...
	arcpy.CreateFeatureDataset_management(handoffGDBpath, "Sections", WebMercator)
	arcpy.AddMessage('/"Sections" feature dataset created')
	arcpy.FeatureClassToFeatureClass_conversion(OldePublish_BoundaryLines2, handoffGDBpath+'/Sections/','SectionBoundaries')
...
# If they exist, clip the section boundaries to the state clipping boundary, reading from OldePublishGDB and writing to HandoffGDB
OldePublish_SectionAreas2 = OldePublishGDBpath+'/Sections/Section_Areas'
Handoff_SectionAreas2 = handoffGDBpath+'/Sections/Section_Areas'

if arcpy.Exists(OldePublish_SectionAreas2):
	arcpy.CreateFeatureDataset_management(handoffGDBpath, "Sections", WebMercator)
	arcpy.AddMessage('/"Sections" feature dataset created')
	arcpy.FeatureClassToFeatureClass_conversion(OldePublish_SectionAreas2, handoffGDBpath+'/Sections/','SectionAreas')
...
Wilderness_Lines_HandoffGDB = handoffGDBpath+'/Wilderness/'+State+'_Wilderness_Boundaries'
Wilderness_Lines_OldePublishGDB = oldExtractsDirectory+OldePublishGDBpath+'/Wilderness/'+State+'_Wilderness_Boundaries'
...
Wilderness_Areas_HandoffGDB = handoffGDBpath+'/Wilderness/'+State+'_Wilderness_Areas'
Wilderness_Areas_OldePublishGDB = OldePublishGDBpath+'/Wilderness/'+State+'_Wilderness_Areas'
# If they exist, import the wilderness lines and areas into the HandoffGDB from the GIS drive
